...
namespace IronBasic.Runtime.Types
{
    public sealed class MbfDouble : MbfFloat
    {
        ...
        public static readonly MbfDouble Ten = FromBytes(0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x84);
        ...
        public MbfDouble(bool isNegitive = false, long mantisa = 0, byte exponent = 0)
        ...
        {
            var single = new MbfDouble(value < 0, Math.Abs(value) << 8, TrueBias + 24);
            single.Normalize();
            return single;
        }
        ...
        public static MbfDouble FromBytes(byte b1, byte b2, byte b3, byte b4, byte b5, byte b6, byte b7, byte b8)
        {
            ...
            var mantisa = (long)Math.Pow((b7 | 0x80) * 0x100, ByteSize - 2);

            // 0, 1
            mantisa += b1;
            mantisa += (byte)(b2 * 0x100);
            mantisa += (byte)(b3 * Math.Pow(0x100, 2));
            mantisa += (byte)(b4 * Math.Pow(0x100, 3));
            mantisa += (byte)(b5 * Math.Pow(0x100, 4));
            mantisa += (byte)(b6 * Math.Pow(0x100, 5));
        ...
        {
            var bytes = ToBytes();
            var mantisa = (long)Math.Pow((bytes[6] | 0x80) * 0x100, ByteSize - 2);
